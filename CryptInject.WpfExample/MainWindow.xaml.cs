...
namespace CryptInject.WpfExample
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            EncryptionManager.Keyring.KeyringChanged += () => listItems.Dispatcher.Invoke(() =>
            {
            ...
            if (File.Exists("data.dat"))
            {
                using (var fs = new FileStream("data.dat", FileMode.Open))
                {
                    ...
                    bf.Binder = new EncryptionProxySerializationBinder();
                    Patients = (ObservableCollection<Patient>)bf.Deserialize(fs);
                }
            }
        }

        private void Keyring_Management_Click(object sender, RoutedEventArgs e)
        {
            var keyringSelector = new KeyringSelection();
            keyringSelector.Show();
        }

        private void Generate_Data_Click(object sender, RoutedEventArgs e)
        {
            Patients.Clear();
